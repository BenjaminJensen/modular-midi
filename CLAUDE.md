...
GitHub Actions (`.github/workflows/ci.yml`) runs on every PR into `main` and every push to `main`: firmware build and host tests, inside the same `docker/Dockerfile` image used locally (see Build section above). A native Linux runner has no WSL2 boundary, so the local Docker slowdown noted there shouldn't apply here.
...
**Linting is not currently part of CI.** There used to be a diff-based clang-tidy step (`tools/lint-diff.sh`, run locally via `lint.ps1`) that only checked lines changed relative to `origin/main` — deliberately, since the pre-refactor HAL code (see Architecture below) already has a large volume of findings and a repo-wide gate would force either a big upfront cleanup or a pile of `NOLINT` suppressions. Both were removed once `tools/clang-tidy.py` replaced the directory-scanning tool they were built around, and a replacement hasn't landed yet — run `tools/clang-tidy.py` manually against touched files in the meantime (see Linting above). The `build-test-lint` job name is a holdover from when it did; don't take the name as confirmation that linting still runs.

**Branch protection is enabled on `main`**: the `build-test-lint` check must pass — currently build+test only, see above — a PR is required (direct pushes are rejected, `enforce_admins` is on so this applies to the repo owner too — verified by attempting a direct push and confirming GitHub rejects it), and linear history is required (GitHub's plain "Merge commit" button is disabled; use "Squash and merge" or "Rebase and merge"). This is what actually enforces the Git workflow rule above, not just convention.
...
`src/shared/hal/pin_concept.h` (`GpioPin`), `src/shared/hal/rp2350/pin.h` (`Pin`), `src/shared/hal/log_sink_concept.h` (`LogSink`), `src/shared/hal/rp2350/rtt_sink.h/.cpp` (`RttSink`), `src/shared/hal/task_runner_concept.h` (`TaskRunner` — a scheduling/execution-context concept rather than a peripheral one; see `architecture/SERVICES.md`), and `src/shared/button.h`/`src/shared/services/button_service.h`/`src/shared/logger.h` (`Button<PinT, SinkT>`/`ButtonService<PinT, SinkT, RunnerT>`/`Logger<SinkT>`) now follow that design. The rest of `src/shared/hal/rp2350/` (`display.h/.cpp`, `st7789.h/.cpp`) still predates it and uses classic runtime polymorphism / non-templated classes. It compiles and runs correctly on target — pico-sdk, FreeRTOS, and lvgl are all in a working integrated configuration — but is a refactor target, not a reference implementation to copy from. When adding new HAL code, follow `architecture/HAL.md`'s concepts-based pattern — `pin_concept.h`/`Pin`/`Button` is a working example of it — rather than mirroring the display code.
